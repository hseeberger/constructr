...
final class Settings private (system: ExtendedActorSystem) extends Extension {
  ...
  object coordination {
    ...
    val backend: Coordination.Backend = {
      constructr.getString("coordination.backend").toLowerCase match {
        case "etcd" => Coordination.Backend.Etcd
        case other  => throw new IllegalArgumentException(s"Unknown coordination backend $other!")
      }
    }

    val host: String = constructr.getString("coordination.host")

    val port: Int = constructr.getInt("coordination.port")
  }
  ...
  val ttlFactor: Double = {
    val ttlFactor = constructr.getDouble("ttl-factor")
    require(
  ...
  private def constructr = system.settings.config.getConfig("constructr")

  private def getDuration(key: String) = FiniteDuration(constructr.getDuration(key).toMillis, MILLISECONDS)
}
